...
@section('content')
    ...
    <div class="d-flex justify-content-start">
        @if (auth()->user()->division == 'FABRIC SALES' && auth()->user()->position_job_name == 'FABRIC SALES EXECUTIVE')
            <a href="{{ route('OutReqKd4') }}" class="btn btn-warning mb-3 mr-2">Outstanding Request<span
        ...
        @elseif (auth()->user()->division == 'FINANCE' && auth()->user()->jabatan == 'Division Manager')
            <a href="{{ route('OutReqKd4') }}" class="btn btn-warning mb-3 mr-2">Outstanding Request<span
                    class="badge badge-info right ml-2">{{ $dmFin }}</span></a>
        @elseif (auth()->user()->position_job_name == 'DIREKTUR FINANCE, ACCT & IT' && auth()->user()->jabatan == 'Direksi')
            <a href="{{ route('OutReqKd4') }}" class="btn btn-warning mb-3 mr-2">Outstanding Request<span
                    class="badge badge-info right ml-2">{{ $gmFin }}</span></a>
        @elseif (auth()->user()->division == 'INTERNAL AUDIT')
            <a href="{{ route('OutReqKd4') }}" class="btn btn-warning mb-3 mr-2">Outstanding Request<span
                    class="badge badge-info right ml-2">{{ $doc }}</span></a>
        @else
            <a href="{{ route('OutReqKd4') }}" class="btn btn-warning mb-3 mr-2">Outstanding Request</a>
        @endif

        <a href="{{ route('kd4req.inputted') }}" class="btn btn-success mb-3 mr-2">Inputted Request</a>
        <a href="{{ route('kd4req.finished') }}" class="btn btn-primary mb-3 mr-2">Finished Request</a>
        <a href="{{ route('kd4req.reject') }}" class="btn btn-danger mb-3 mr-2">Rejected Request</a>
    </div>
</div>
    <hr>
                            ...
                            <tbody>
                                @foreach ($kd04 as $form)
                                    <tr>
                                        ...
                                        <td>
                                            @if (auth()->user()->division == 'FABRIC SALES' && auth()->user()->position_job_name == 'FABRIC SALES EXECUTIVE')
                                                <a href="/dashboard/salesFolder/change/request/show/kd4/{{ $form->id }}"
