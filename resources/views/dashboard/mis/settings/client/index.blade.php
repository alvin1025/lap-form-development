...
@section('content')
    ...
    <div class="table-responsive">
        ...
        <div class="d-flex justify-content-start">
            <a href="{{ route('client.create') }}" class="btn btn-primary mb-3">Create New Form</a>
        ...
        <table class="table table-light table-striped" id="dt-init">
            ...
            <tbody>
                @foreach ($client as $form)
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                        <td>{{ $form->key }}</td>
                        <td>{{ $form->description }}</td>
                        <td>
                            <a href="/dashboard/mis/settings/client/{{ $form->id }}" class="badge bg-info"><i
                                    class="fa fa-eye" aria-hidden="true"></i></a>
                            <a href="/dashboard/mis/settings/client/{{ $form->id }}/edit" class="badge bg-warning"><i
                                    class="fas fa-edit"></i></a>
                            <form action="/dashboard/mis/settings/client/{{ $form->id }}" method="post"
                                class="d-inline">
                                @method('delete')
                                @csrf
