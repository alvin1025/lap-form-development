...
@section('content')
    ...
    <div class="table-responsive">
        ...
        <div class="d-flex justify-content-start">
            <a href="{{ route('pr03.create') }}" class="btn btn-primary mb-3">Create New Form</a>
        ...
        <table class="table table-light table-striped" id="dt-init">
            <thead>
                <tr>
                    ...
                    <th scope="col">PriceGrp</th>
                    <th scope="col">ACTION</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($pr03 as $form)
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                        <td>{{ $form->coll }}</td>
                        <td>{{ $form->pricegrp }}</td>
                        <td>
                            <a href="/dashboard/mis/settings/pr03/{{ $form->id }}" class="badge bg-info"><i
                                    class="fa fa-eye" aria-hidden="true"></i></a>
                            <a href="/dashboard/mis/settings/pr03/{{ $form->id }}/edit" class="badge bg-warning"><i
                                    class="fas fa-edit"></i></a>
                            <form action="/dashboard/mis/settings/pr03/{{ $form->id }}" method="post"
                                class="d-inline">
                                @method('delete')
                                @csrf
