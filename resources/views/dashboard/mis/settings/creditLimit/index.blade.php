...
@section('title', 'Form Credit Limit Data')
...
@section('content')
    ...
    <div class="table-responsive">
        ...
        <div class="d-flex justify-content-start">
            <a href="{{ route('creditLimit.create') }}" class="btn btn-primary mb-3">Create New Form</a>
            ...
            <div class="modal fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <form action="{{ route('importCreditlimit') }}" method="post" enctype="multipart/form-data">
                            @csrf
        ...
        <table class="table table-light table-striped" id="dt-init">
            ...
            <tbody>
                @foreach ($creditLimit as $form)
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                        <td>{{ $form->credit_limit }}</td>
                        <td>{{ $form->description }}</td>
                        <td>
                            <a href="/dashboard/mis/settings/creditLimit/{{ $form->id }}" class="badge bg-info"><i
                                    class="fa fa-eye" aria-hidden="true"></i></a>
                            <a href="/dashboard/mis/settings/creditLimit/{{ $form->id }}/edit"
                                class="badge bg-warning"><i class="fas fa-edit"></i></a>
                            <form action="/dashboard/mis/settings/creditLimit/{{ $form->id }}" method="post"
                                class="d-inline">
                                @method('delete')
                                @csrf
