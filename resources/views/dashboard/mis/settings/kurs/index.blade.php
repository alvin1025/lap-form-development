...
@section('content')
    ...
    <div class="table-responsive">
        ...
        <div class="d-flex justify-content-start">
            <a href="{{ route('kurs.create') }}" class="btn btn-primary mb-3">Create New Form</a>
        ...
        <table class="table table-light table-striped" id="dt-init">
            ...
            <tbody>
                @foreach ($kurs as $form)
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                        <td>{{ $form->kurs }}</td>
                        <td>{{ $form->description }}</td>
                        <td>
                            <a href="/dashboard/mis/settings/kurs/{{ $form->id }}" class="badge bg-info"><i
                                    class="fa fa-eye" aria-hidden="true"></i></a>
                            <a href="/dashboard/mis/settings/kurs/{{ $form->id }}/edit" class="badge bg-warning"><i
                                    class="fas fa-edit"></i></a>
                            <form action="/dashboard/mis/settings/kurs/{{ $form->id }}" method="post"
                                class="d-inline">
                                @method('delete')
                                @csrf
