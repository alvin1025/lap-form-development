...
@section('content')
    ...
    <div class="table-responsive">
        ...
        <div class="d-flex justify-content-start">
            <a href="{{ route('orientasiMarket.create') }}" class="btn btn-primary mb-3">Create New Form</a>
        ...
        <table class="table table-light table-striped" id="dt-init">
            <thead>
                <tr>
                    ...
                    <th scope="col">Orientasi Market</th>
                    <th scope="col">Descriptions</th>
                    <th scope="col">ACTION</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($orientasiMarket as $form)
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                        <td>{{ $form->orientasi_market }}</td>
                        <td>{{ $form->description }}</td>
                        <td>
                            <a href="/dashboard/mis/settings/orientasiMarket/{{ $form->id }}" class="badge bg-info"><i
                                    class="fa fa-eye" aria-hidden="true"></i></a>
                            <a href="/dashboard/mis/settings/orientasiMarket/{{ $form->id }}/edit"
                                class="badge bg-warning"><i class="fas fa-edit"></i></a>
                            <form action="/dashboard/mis/settings/orientasiMarket/{{ $form->id }}" method="post"
                                class="d-inline">
                                @method('delete')
                                @csrf
